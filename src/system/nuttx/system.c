...
//
 ...
 /* Use NuttX's C library types for time and clock */
 typedef struct timespec z_clock_t;
 typedef struct timeval  z_time_t;
 
 /* Thread types (using POSIX pthreads) */
 ...
 {
   // With pthreads, the task handle is not dynamically allocated,
   // so simply set the pointer to NULL.
   *task = NULL;
 }
